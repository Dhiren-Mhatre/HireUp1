...
def Matching():
    ...
    if resume_data is None:
        print("No resume data found for the user.")
        return 0  # or handle this case appropriately

    resume_workedAs = resume_data.get("WORKED AS", [])
    print("resume_workedAs:", resume_workedAs)

    resume_experience_list = resume_data.get("YEARS OF EXPERIENCE", None)

    # Set default experience to 1 if no experience data is found
    if resume_experience_list is None:
        print("No experience data found for the user. Setting default experience to 1.")
        resume_experience_list = [1]  # Default experience of 1 year
    else:
        resume_experience_list = [int(exp) for exp in resume_experience_list]  # Ensure it's a list of integers
    ...
    for p in resume_experience_list:
        parts = str(p).split()
        if "years" in str(p) or "year" in str(p):
            year = int(parts[0])
            if "months" in str(p) or "month" in str(p):
                year += int(parts[2]) / 12
        else:
            year = int(parts[0]) / 12
        year = round(year, 2)
