from flask import Flask, render_template, url_for, request, session, redirect, abort, jsonify,make_response
...
app.config['MONGO_URI']= 'mongodb://localhost:27017/HireUp' 
...
@app.route('/login')
def login():
    session.clear()  # Clear session before starting a new login flow
...
def logout():
    ...
    response.delete_cookie('session')  # Deletes session cookie
    response.delete_cookie('another_cookie')
